...
class SSEBlock(nn.Module):
    ...
    def forward(self, inputs):
        custom_print(inputs, 'SSEBlockStart')
...
class Downsample(nn.Module):
    ...
    def forward(self, inputs):
        custom_print(inputs, 'DownsampleStart')
        ...
        custom_print(x, 'DownsampleStart_x')
        custom_print(y, 'DownsampleStart_y')
        custom_print(z, 'DownsampleStart_z')
        a = x + y
        b = torch.mul(a, z)
        c = self.activation(b)
        custom_print(c, 'DownsampleEnd')
        return c
...
class Fusion(nn.Module):
    ...
    def forward(self, input1, input2):
        custom_print(input1, 'FusionStart1')
        custom_print(input2, 'FusionStart2')
...
class Stream(nn.Module):
    ...
    def forward(self, inputs):
        custom_print(inputs, 'StreamStart')
        a = self.sse(inputs)
        b = self.fuse(inputs)
        c = a + b

        d = self.activation(c)
        custom_print(d, 'StreamEnd')
        return d
...
def custom_print(inputs, name):
    print(f'Called from {name}, size is {inputs.size()}')
